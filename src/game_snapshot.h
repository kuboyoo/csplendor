...
public:
  ...
  void bytes(std::string_view value) { data_.append(value); }
...
inline uint64_t fingerprint_bytes(uint64_t hash, uint8_t value) {
  hash ^= static_cast<uint64_t>(value);
  return hash * 1099511628211ULL;
}

inline uint64_t ruleset_fingerprint() {
...
inline uint64_t payload_checksum(std::string_view payload) {
  uint64_t hash = 14695981039346656037ULL;
...
inline bool valid_card_id(int value) {
  return value >= 0 && value < CARD_COUNT;
}

inline bool valid_noble_id(int value) {
  return value >= 0 && value < NOBLE_COUNT;
}

inline void require_boolean(uint8_t value, const char *field) {
  if (value > 1)
    throw std::invalid_argument(
        std::string("invalid boolean in game snapshot: ") + field);
}

inline std::string serialize(const Game &game) {
  Writer payload;
...
inline Game deserialize(std::string_view snapshot) {
