...
import json
import sys
from phase3d1_record_20260905 import ROOT, BASE, BASE_SOURCE, RELEASE, CASES, run, save
...
def build():
    for label, flags in (
        ...
        ('reference-diagnostic', ['-DCSPLENDOR_PERF_INSTRUMENTATION=ON', '-DCSPLENDOR_SOLVER_NORMAL_ROLLBACK=OFF']),
        ('verify', ['-DCSPLENDOR_VERIFY_SOLVER_ROLLBACK=ON', '-DCSPLENDOR_VERIFY_REVEAL_SCORE_ORDER=ON', '-DCSPLENDOR_VERIFY_REVEAL_SEARCH_STATE=ON', '-DCSPLENDOR_VERIFY_INCREMENTAL_HASH=ON']),
    ):
        directory = ROOT / ('build/3d1-' + label)
        ...
        checked('build_' + label, ['cmake', '--build', str(directory), '-j4',
            '--target', 'benchmark_engine_hotpaths', 'solver_components_unit', 'solver_normal_rollback_unit'])
        checked('unit_' + label, [str(directory / 'tests/solver_components_unit')])
        checked('rollback_unit_' + label, [str(directory / 'tests/solver_normal_rollback_unit')])
